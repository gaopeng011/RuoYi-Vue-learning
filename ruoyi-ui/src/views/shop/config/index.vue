<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="系统配置名" prop="keyName">
        <el-input
      ...
      <el-form-item label="系统配置值" prop="keyValue">
        <el-input
    ...
    <el-table v-loading="loading" :data="systemList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="系统配置名" align="center" prop="keyName" />
      <el-table-column label="系统配置值" align="center" prop="keyValue" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="系统配置名" prop="keyName">
          <el-input v-model="form.keyName" placeholder="请输入系统配置名" />
        </el-form-item>
        <el-form-item label="系统配置值" prop="keyValue">
          <el-input v-model="form.keyValue" placeholder="请输入系统配置值" />
        </el-form-item>
        <el-form-item label="创建时间" prop="addTime">
          <el-date-picker clearable size="small" style="width: 200px"
...
<script>
import { listSystem, getSystem, delSystem, addSystem, updateSystem, exportSystem } from "@/api/shop/config";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        keyName: [
          { required: true, message: "系统配置名不能为空", trigger: "blur" }
        ],
        keyValue: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除系统配置编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有系统配置数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
